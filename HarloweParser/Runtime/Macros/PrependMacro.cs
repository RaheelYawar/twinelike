...
{
  /// <summary>
  /// <c>(prepend: ?target, …)[extra content]</c>. A revision changer: the
  /// attached hook is rendered into a detached subtree and spliced in
  /// <em>before</em> the existing content of every node each target resolves
  ...
  public class PrependMacro : IMacro
  {
    ...
    public HarloweValue Invoke(List<HarloweValue> args, MacroContext context)
      => RevisionChangers.Build(args, RevisionMode.Prepend, "(prepend:)");
  }
}
