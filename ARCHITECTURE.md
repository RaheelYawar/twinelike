...
Two report APIs exist for the same reason: the engine is deliberately tolerant, so nothing *forces* an authoring mistake to surface. Inline errors only fire when a player reaches them, and a broken passage doesn't abort a load. A typo down an unplayed branch therefore stays invisible until it ships. Both are meant to be called once at load by a host engine, which shows the results to whoever is building the story — so each entry carries its passage, line, column, and a ready-to-print `Message`.
...
Both walk the AST through internal visitors in `BodyVisitors.cs` (`ReferenceCollector`, `ParseErrorCollector`), which is why `LinkNode`, `MacroNode`, and `ParseErrorNode` carry source positions — the diagnostic has to be able to point at something.
